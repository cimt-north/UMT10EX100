...
begin
  ...
  SpeedButtonClear.Caption  := 'Clear';
...
begin
  ...
  try
    ...
    // <-- ใช้ชื่อคงที่ ไม่มีวันที่
    Ini.WriteString('Output1','Path','C:\Export\NewOrder_Job.csv');
    Ini.WriteString('Output2','Path','C:\Export\NewOrder_Part.csv');
    Ini.WriteString('Output3','Path','C:\Export\NewOrder_Process.csv');
...
begin
  try
    ...
    SpeedButtonExport.Enabled := false;
...
procedure TUMT10EX100.SpeedButtonClearClick(Sender: TObject);
begin
  // Clear all memo logs
  Memo1.Clear;
  ...
  SpeedButtonExport.Enabled := True;
  SpeedButtonSetting.Enabled := True;

  // Update status bar last panel (time)
  if StatusBar1.Panels.Count > 0 then
    StatusBar1.Panels[StatusBar1.Panels.Count - 1].Text := '';

  // Optional visual feedback
  Memo1.Lines.Add('Cleared all logs and reset progress.');
end;
