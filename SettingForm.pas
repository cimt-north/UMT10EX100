...
begin
  ...
  try
    ...
    // บังคับชื่อไฟล์คงที่ .csv (ไม่เอาวันที่)
    if outJobDir  <> '' then
      fullJob  := TPath.Combine(outJobDir , 'NewOrder_Job.csv') else fullJob := '';
    if outPartDir <> '' then
      fullPart := TPath.Combine(outPartDir, 'NewOrder_Part.csv') else fullPart := '';
    if outProcDir <> '' then
      fullProc := TPath.Combine(outProcDir, 'NewOrder_Process.csv') else fullProc := '';

    // เขียนลง INI
    Ini.WriteString('Output1', 'Path', fullJob);
    Ini.WriteString('Output2', 'Path', fullPart);
    Ini.WriteString('Output3', 'Path', fullProc);
